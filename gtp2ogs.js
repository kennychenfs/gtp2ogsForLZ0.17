...
if (argv.fast1) {
  FAST1 = Number(argv.fast1);
}

if (argv.fast2) {
  FAST2 = Number(argv.fast2);
}
...
class Bot {
    ...
    postToChat(errline) {
     /* 
	 H12 ->    1192 (V: 45.78%) (LCB: 43.80%) (N: 44.70%) PV: H12 H13 G13 L13 R4 R5 K14 J9 F12 O5 J2 C7 B7 B8 B9 B6
	 J14 ->       1 (V: 20.35%) (LCB:  0.00%) (N:  0.04%) PV: J14 
	  G5 ->       1 (V: 13.78%) (LCB:  0.00%) (N:  0.06%) PV: G5 
	 L13 ->       1 (V: 13.27%) (LCB:  0.00%) (N:  0.02%) PV: L13 
	  G6 ->       1 (V: 11.68%) (LCB:  0.00%) (N:  0.02%) PV: G6 
	  F7 ->       1 (V: 10.83%) (LCB:  0.00%) (N:  0.03%) PV: F7 
	  E8 ->       1 (V:  9.82%) (LCB:  0.00%) (N:  0.02%) PV: E8 
	  S5 ->       1 (V:  9.46%) (LCB:  0.00%) (N:  0.02%) PV: S5 
	 K14 ->       1 (V:  9.38%) (LCB:  0.00%) (N:  0.02%) PV: K14 
	  S4 ->       1 (V:  6.71%) (LCB:  0.00%) (N:  0.02%) PV: S4 
	  S3 ->       1 (V:  6.05%) (LCB:  0.00%) (N:  0.02%) PV: S3 
	  E7 ->       1 (V:  5.48%) (LCB:  0.00%) (N:  0.02%) PV: E7 
	  J9 ->       1 (V:  3.46%) (LCB:  0.00%) (N:  0.10%) PV: J9 
	  T4 ->       1 (V:  1.89%) (LCB:  0.00%) (N:  0.02%) PV: T4 
	  T3 ->       1 (V:  1.24%) (LCB:  0.00%) (N:  0.02%) PV: T3 
	 S19 ->       1 (V:  0.29%) (LCB:  0.00%) (N:  0.02%) PV: S19 

	16.1 average depth, 44 max depth
	29765 non leaf nodes, 1.39 average children

	41479 visits, 8946654 nodes, 10589 playouts, 1027 n/s
      */
        if (!this.prevLine) {
  // R5 ->      59 (V: 99.89%) (N:  6.84%) PV: R5 S6 Q7 Q6 Q5 R6 P6 T4 T3 T5
            let myPVRe = /([A-Z]\d+|pass) -> +(\d+) \(V: +(\d+.\d\d)%\) \(LCB: +(\d+.\d\d)%\) \(N: +(\d+.\d\d)%\) PV:(( ([A-Z][0-9]+|pass)+)+)/;
...
// 1048 visits, 151967 nodes, 974 playouts, 166 n/s
        ...
        if (stop && this.prevLine) {
            let winrate = this.prevLine[3],
			  intuition = this.prevLine[5],
              visits = stop[1],
              playouts = stop[3],
              nps = stop[4],
              pv = this.prevLine[6];
            this.winrate = Number(this.prevLine[3]);
            let body = this.createMessage(winrate, visits, playouts, nps, pv, intuition);
            this.game.sendChat(body, this.game.state.moves.length+1, "malkovich");
            this.prevLine = null;
        }
    }
    
    createMessage(winrate, visits, playouts, nps, pv, intuition) {
        let rawmoves = pv.trim().split(" ");
        return {
            "type": "analysis",
            "name": `Winrate: ${winrate}%, Visits: ${visits}, Intuition: ${intuition}% for this point.`,
    ...
    loadClock(state) {
        ...
        //
        this.command("time_settings 0 11 1");
        return;
